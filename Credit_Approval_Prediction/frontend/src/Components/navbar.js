...
import { faSignInAlt, faCog } from '@fortawesome/free-solid-svg-icons'; // Import faCog icon for settings
...
const Navbar = () => {
  ...
  const fetchUserData = async (username) => {
    try {
      const response = await axios.get(`http://127.0.0.1:4000/register/${username}`);
      setUserData(response.data);
      setIsLogged(true); // Set isLogged to true when user data is fetched
    } catch (error) {
      console.error('Error fetching user data:', error);
    }
  };
  ...
  const handleSignout = () => {
    ...
    if (confirmSignout) {
      ...
      localStorage.clear();
      setUserData(null);
      setIsLogged(false); // Set isLogged to false on sign out
      navigate('/');
    }
  };
  ...
  return (
    <nav className="navbar navbar-expand-lg navbar-light fixed-top">
      <div className="container-fluid">
        ...
        {/* Use the logo image as the content of the Link */}
        <img src={LogoImage} alt="Creditready Logo" className="logo-img" />
        ...
        <div className="collapse navbar-collapse" id="navbarNav">
          <ul className="navbar-nav ms-auto">
            <li className="nav-item">
              ...
              {!isLogged && (
                <Link to="/login" className="nav-link sign-icon">
